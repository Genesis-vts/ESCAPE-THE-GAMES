...
- [ ] `clinical`: definir **qual instrumento em qual momento da jornada** — o NODS-3-BR tem acurácia diagnóstica publicada mas população errada; o OGD-Q BR tem a população certa mas sem sensibilidade/especificidade `TODO [CLINICAL]`
- [ ] `research`: resolver a **contradição de prevalência** — Laranjeira et al. 2025 (17,6% apostaram online, 7,3% problemático) × LENAD III (6,8% risco, 0,8% transtorno). Escopo, método e definição diferem — não citar os dois lado a lado antes de resolver
- [ ] `product`: usar a taxonomia de **dark nudges** do TCC da UFSC para escrever a seção de anti-padrões — o que o produto nunca faz é, em boa medida, o inverso do catálogo da indústria
...
- [ ] `research`: ler a tese de **Silvia Saboia Martins** (jogo patológico no gênero feminino) na fonte primária e verificar se o **efeito telescópio** (~4 anos em mulheres vs. ~14 em homens) foi replicado depois da aposta online — [DATA_SOURCES.md §4.1.2](./DATA_SOURCES.md)
- [ ] `product`: revisar a persona do `MVP_SPEC.md` — hoje é masculina, e a janela de intervenção pode ser ~3,5× mais curta para mulheres; depende do item acima antes de mudar qualquer coisa
- [ ] `product`: **o botão de pânico pede acionamento no pior momento possível** — a evidência mostra que a perda é o sinal de _continuar_, não de parar ([DATA_SOURCES.md §4.3](./DATA_SOURCES.md)). Desenhar acionamento que não dependa só da iniciativa no estado quente (pré-compromisso, gatilho do familiar, telemetria)
- [ ] `research`: avaliar **latência entre apostas e parada pós-resultado** como sinal de telemetria — mensurável sem conteúdo nem valores, compatível com o princípio P3 de minimização
